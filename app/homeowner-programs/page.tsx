...
const indexLinks: { href: string; label: string; detail: string }[] = [
	{
		href: '#harden',
		label: 'Harden your home',
		detail: 'My Safe Florida Home grants, condo pilot, weatherization',
	},
	{
		href: '#disputes',
		label: 'Insurance disputes',
		detail: 'Free state mediation and the DFS consumer helpline',
	},
	{
		href: '#repair-money',
		label: 'Repair & disaster money',
		detail: 'SHIP, USDA loans and grants, FEMA and SBA aid',
	},
	{
		href: '#taxes-relief',
		label: 'Property-tax & other relief',
		detail: 'Homestead, storm-damage tax refunds, veteran grants',
	},
	{
		href: '#treasure-coast',
		label: 'Treasure Coast',
		detail: 'Indian River, St. Lucie & Martin county programs',
	},
	{
		href: '#space-coast',
		label: 'Space Coast',
		detail: 'Brevard County and its city repair programs',
	},
	{
		href: '#orlando-area',
		label: 'Orlando area',
		detail: 'Orange, Osceola, Seminole & Lake county programs',
	},
]
	...
	return (
		<>
			<Hero
				...
				support="The complete guide to government help for your home — statewide grants, free claim mediation, property-tax relief, and county-by-county repair programs across the Treasure Coast, Space Coast, and Orlando area. Every link, phone number, and current status."
			...
			<section id="index" className="scroll-mt-24 py-16 sm:py-20">
				<div className="section-shell space-y-10">
					<SectionHeading
						...
						description="Jump straight to the help you need. Statewide programs apply everywhere in Florida; the regional sections list every county office, phone number, and whether applications are open right now. Statuses verified August 2026 — funding windows open and close quickly, so always call before you count on a program."
					/>
					<ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
						{indexLinks.map((link) => (
							<li key={link.href}>
								<a
									href={link.href}
									className="block h-full rounded-2xl border border-navy/10 bg-white p-6 transition-colors hover:border-sand"
								>
									<span className="font-display text-xl text-navy">{link.label}</span>
									<span className="mt-2 block font-sans text-sm text-stone-muted">
										{link.detail}
									</span>
								</a>
							</li>
			...
			{buckets.map((bucket) => (
				<section key={bucket.id} id={bucket.id} className="scroll-mt-24 py-20 sm:py-24">
			...
			{regions.map((region, regionIndex) => (
				<section
					key={region.id}
					id={region.id}
					className={`scroll-mt-24 py-20 sm:py-24 ${regionIndex % 2 === 0 ? 'surface-atmosphere' : ''}`}
				>
