export const siteConfig = {
	...
	/**
	 * Public social profiles. Verified live on 30 Aug 2026 before being linked.
	 *
	 * ONLY profiles with real activity belong here. A homeowner deciding whether
	 * to trust a contractor with tens of thousands of dollars clicks these; an
	 * empty channel reads as a company that started things and stopped, which is
	 * worse than no link at all.
	 *
	 * Deliberately NOT listed, and why (re-check before adding):
	 *   LinkedIn linkedin.com/company/covenantbuildersorg — exists, 1 follower,
	 *     generic boilerplate copy that contradicts the voice of this site.
	 *   YouTube  youtube.com/@covenantbuildersorg — exists, 1 subscriber,
	 *     zero videos ("This channel doesn't have any content").
	 *   X        x.com/covenantbuilde — exists, one post from May 2025, a joke.
	 *   TikTok   tiktok.com/@covenantbuildersorg — could not be verified.
	 *
	 * Note the Instagram handle ends in **s**. `covenantbuildersorg` without the
	 * trailing s is not a real account, and Facebook's own linked-accounts panel
	 * displays it wrongly — trust the profile URL, not that panel.
	 */
	social: [
		{
			name: 'Facebook',
			href: 'https://www.facebook.com/covenantbuildersorg',
		},
		{
			name: 'Instagram',
			href: 'https://www.instagram.com/covenantbuildersorgs/',
		},
	] as const,
	responseExpectation:
